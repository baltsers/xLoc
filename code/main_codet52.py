...
class TextDataset(Dataset):
    ...
    def __getitem__(self, i):       
        ...
        for position in self.relative_call_locations[i]:
            try:
                relative_call_position[position[0]][position[1]] = self.relative_call_locations[i][position]
            except:
                break
...
def test(args, model, tokenizer, test_dataset, best_threshold=0.5):
    ...
    df.to_csv("./raw_predictions/VulRepair_raw_preds_ori__beam"+str(len(beam_outputs))+".csv")
...
def main():
    ...
    parser.add_argument("--model_name", default="model_codet5_fc_as.bin", type=str,
